// GridRenderer3D.js — Canvas 2D + 3D projection renderer for Minesweeper 3D
// Port of Client/src/ui/cpp/GridRenderer3D.cpp
// ============================================================================

import { CellContent, CellState, GameState, ClickType } from '../../core/CoreEnums.js';
...
function getCellGlyph(cell, paintFalseFlags) {
...
export class GridRenderer3D {
    ...
    setInert(inert) {
        ...
        if (inert) this._stopKeyboardRotate();
    }
    ...
    hide() { this._canvas.style.display = 'none'; this._inert = true; this._hasKeyboardSelection = false; }
        ...
        for (const fo of faceOrder) {
            ...
            for (let r = 0; r < rows; r++) {
                for (let c = 0; c < cols; c++) {
                    ...
                    const glyph = getCellGlyph(cell, this._paintFalseFlags);
                    if (glyph) {
                        const inset = 0.1;
    ...
    _setupInput() {
        ...
        c.addEventListener('mouseup', (e) => {
            if (this._inert) return;
            clearTimeout(this._longPressTimer);
            if (!this._isRightClick && !this._hasMoved && !this._longPressTriggered && !this._isPinching) {
                const pos = this._getCanvasPos(e);
                const hit = this._hitTest(pos.x, pos.y);
        ...
        c.addEventListener('touchmove', (e) => {
            ...
            if (touches.length === 1 && this._dragging && !this._isPinching) {
                ...
                if (!this._hasMoved) {
                    if (Math.sqrt(ddx*ddx + ddy*ddy) > CLICK_THRESHOLD) {
                        this._hasMoved = true;
                        clearTimeout(this._longPressTimer);
                    }
                }
